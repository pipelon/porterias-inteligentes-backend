...
$template = '';
if (\Yii::$app->user->can('/cities/view')) {
    $template .= '{view} ';
}
if (\Yii::$app->user->can('/cities/update')) {
    $template .= '{update} ';
}
if (\Yii::$app->user->can('/cities/delete')) {
    $template .= '{delete} ';
}
if (\Yii::$app->user->can('/cities/*') || \Yii::$app->user->can('/*')) {
    $template = '{view}  {update}  {delete}';
}
?>
<div class="cities-index box box-primary">
    <div class="box-header with-border">
    <?php  if (\Yii::$app->user->can('/cities/create') || \Yii::$app->user->can('/*')) :  ?> 
        <?= Html::a('<i class="flaticon-add" style="font-size: 20px"></i> '.'Crear ciudad', ['create'], ['class' => 'btn btn-primary']) ?>
    <?php  endif;  ?> 
    </div>
